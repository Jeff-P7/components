 ...
 * @return {*}  {boolean}
 */
const isSpacingValid: Function = (val: string | number): boolean => {
  if (val && !SpacingTypes.includes(val)) throw Error(spacingMessageError(val));
  else if (val && SpacingTypes.includes(val)) return true;
  else return false;
};
